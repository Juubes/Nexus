...
public class CreateMapCommand implements CommandExecutor {
	...
	public CreateMapCommand(Nexus nexus) {
		this.nexus = nexus;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String lbl, String[] args) {
		if (!sender.isOp()) {
			sender.sendMessage("�cEt voi luoda mappia. Op h�x h�x.");
			return true;
		}
		...
		if (args.length == 2) {
			sender.sendMessage("�c/createmap " + mapID + " " + ChatColor.translateAlternateColorCodes('&', displayName)
					+ " <time ticks>");
			return true;
		}
